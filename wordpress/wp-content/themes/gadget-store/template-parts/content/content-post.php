...
<div id="post-<?php the_ID(); ?>" <?php post_class('blog-item'); ?>>
    ...
    if ( is_single() ) :
        ...
        if ( $gadget_store_single_post_feature_image == '1' ) :
            if ( has_post_thumbnail() ) :
                echo '<div class="blog-thumb">' . get_the_post_thumbnail() . '</div>';
            else :
                // Using a default image for posts without a featured image
                $gadget_store_default_image_url = esc_url( get_template_directory_uri() . '/assets/images/default.png' );
                echo '<div class="blog-thumb"><img src="' . $gadget_store_default_image_url . '" alt="' . esc_attr__( 'Default Image', 'gadget-store' ) . '"></div>';
            endif;
        endif;
    else :
        // Archive post settings
        if ( $gadget_store_post_feature_image == '1' ) :
            if ( has_post_thumbnail() ) :
                echo '<div class="blog-thumb">' . get_the_post_thumbnail() . '</div>';
            else :
                // Using a default image for posts without a featured image
