...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			/*
			 * The `.sub-menu` container needs to have a labelledId
			 * attribute which points to it's trigger link.
			 *
			 * Form a string for the labelledId attribute from the the latest
			 * link with an id name that was added to the $output.
			 */
			$labelledId = '';
			// Find all links with an id in the output.
			preg_match_all( '/(<a.*?data-bs-name=\"|\')(.*?)\"|\'.*?>/im', $output, $matches );
			// With pointer at end of array check if we got an ID match.
			if ( end( $matches[2] ) ) {
				// Build a string to use as aria-labelledId.
				$labelledId = 'id="' . esc_attr( end( $matches[2] ) ) . '"';
			}
			$output .= "{$n}{$indent}<ul$class_names $labelledId>{$n}";
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				$atts['data-bs-target']   = '#menu-item-dropdown-' . $item->ID;
				$atts['aria-haspopup']    = 'true';
				$atts['aria-expanded']    = 'false';
				$atts['class']            = 'dropdown-toggle nav-link';
				$atts['data-bs-name']     = 'menu-item-dropdown-' . $item->ID;
			} else {
		...
		public static function fallback( $args ) {
			...
			$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'gadget-store' ) . '">' . esc_html__( 'Add a menu', 'gadget-store' ) . '</a></li>';
			$fallback_output .= '</ul>';
